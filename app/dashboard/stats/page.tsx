...
import {
  ...
  ResponsiveContainer, LineChart, Line, RadialBarChart, RadialBar, Legend,
} from "recharts";
...
export default function StatsPage() {
  ...
  const weekRate = stats.weekTasks > 0
    ? Math.round((stats.weekCompleted / stats.weekTasks) * 100) : 0;
  const monthRate = stats.monthTasks > 0
    ? Math.round((stats.monthCompleted / stats.monthTasks) * 100) : 0;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 py-3 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-gray-900 dark:text-gray-100">Statistiques</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-200 text-sm mt-1">
                {stats.streak === 0 ? "Commence aujourd'hui !" :
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard label="Tâches aujourd'hui" value={`${stats.todayCompleted}/${stats.todayTasks}`} color="blue" />
          <StatCard label="Taux global" value={`${globalRate}%`} color="amber" />
        </div>

        <div className="grid md:grid-cols-2 gap-4">
          {/* 7 derniers jours */}
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4 text-sm">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={stats.last4Weeks}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: "#1f2937", border: "1px solid #374151", borderRadius: "8px", color: "#f9fafb" }}
                />
                <Line type="monotone" dataKey="total" name="Total" stroke="#ddd6fe" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="completed" name="Complétées" stroke="#7c3aed" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <RadialBarChart innerRadius="30%" outerRadius="90%" data={radialData} startAngle={180} endAngle={0}>
                <RadialBar label={{ position: "insideStart", fill: "#fff", fontSize: 11 }} background dataKey="value" />
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
            ...
            {stats.goalsProgress.length === 0 ? (
              <div className="flex items-center justify-center h-40">
                <p className="text-gray-400 dark:text-gray-500 text-sm">Aucun objectif actif</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={stats.goalsProgress} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                  <XAxis type="number" domain={[0, 100]} tick={{ fontSize: 11, fill: "#9ca3af" }} />
                  <YAxis dataKey="title" type="category" tick={{ fontSize: 11, fill: "#9ca3af" }} width={80} />
                  <Tooltip formatter={(value) => `${value}%`}
                    contentStyle={{ backgroundColor: "#1f2937", border: "1px solid #374151", borderRadius: "8px", color: "#f9fafb" }}
                  />
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
            <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-3 text-sm">Cette semaine</h3>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-500 dark:text-gray-400">{stats.weekCompleted}/{stats.weekTasks} tâches</span>
              <span className="text-sm font-medium text-purple-600 dark:text-purple-400">{weekRate}%</span>
            </div>
            <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
              <div className="bg-purple-600 h-2 rounded-full" style={{ width: `${weekRate}%` }} />
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 p-4">
            <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-3 text-sm">Ce mois</h3>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-500 dark:text-gray-400">{stats.monthCompleted}/{stats.monthTasks} tâches</span>
              <span className="text-sm font-medium text-purple-600 dark:text-purple-400">{monthRate}%</span>
            </div>
            <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
              <div className="bg-purple-600 h-2 rounded-full" style={{ width: `${monthRate}%` }} />
            </div>
          </div>
        </div>
      </main>
